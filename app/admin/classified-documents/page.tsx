...
import { Archive, Eye, FileText, PencilLine, Plus } from 'lucide-react'
...
function visibilitySummary(roles: AdminRole[]) {
  const recipients = roles.filter(role => role !== 'P2')
...
export default function ClassifiedDocumentsPage() {
  ...
  const reloadDocs = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [remoteDocs, archivedDocs] = await Promise.all([
        getConfidentialDocs(),
        getArchivedDocs(),
      ])

      const classifiedArchived = (archivedDocs ?? []).filter((item: any) => String(item.type ?? '').toLowerCase() === 'classified document')
      ...
      const loadedDocs = await Promise.all(
        activeDocs.map(async doc => {
          ...
          return { ...doc, visibleRoles }
        })
      )

      setDocs(loadedDocs)
      setArchivedCount(classifiedArchived.length)
    } catch (error) {
      console.error(error)
  ...
  const stats = useMemo(() => {
    const p2Only = docs.filter(doc => doc.visibleRoles.every(role => role === 'P2')).length
  ...
  const filteredDocs = useMemo(() => {
    ...
    return docs.filter(doc => {
      ...
      const forwarded = doc.visibleRoles.some(role => role !== 'P2')
  ...
  async function handleArchive() {
    ...
    const archiveId = `arc-cd-${doc.id}`
    const today = new Date().toISOString().split('T')[0]

    const archived = await archiveConfidentialDoc(doc.id)
    if (!archived) {
      toast.error('Could not archive the classified document.')
      return
    }

    const archiveLogged = await addArchivedDoc({
      id: archiveId,
      title: doc.title,
      type: ARCHIVE_LABEL,
      archivedDate: today,
      archivedBy: user?.role ?? 'P2',
    })

    if (!archiveLogged) {
      toast.error('Archived, but the archive log entry failed.')
    }
  ...
  return (
    <>
      ...
      <div className="p-8 space-y-6">
        {!canManage && (
          <div className="max-w-4xl">
            <AlertWarning message="Classified Documents are reserved for P2." />
          </div>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">Managed directly by P2.</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-3 flex items-end justify-between">
              <p className="text-3xl font-black text-slate-800">{archivedCount}</p>
              <Archive className="h-6 w-6 text-slate-300" />
            </div>
            <p className="mt-2 text-sm text-slate-500">Moved to archived records.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="border-b border-slate-100 px-6 py-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Create, edit, and archive documents from P2.</p>
            </div>
            {canManage && (
                      ...
                      <tr key={doc.id} className="border-b border-slate-100 hover:bg-slate-50/80 transition">
                        <td className="px-5 py-4 align-top">
                          <button type="button" onClick={() => detailDisc.open(doc)} className="flex items-center gap-3 text-left">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-slate-800 hover:text-blue-600">{doc.title}</p>
                              <p className="text-xs text-slate-400">{doc.access}</p>
                            </div>
                          </button>
                        </td>
                        <td className="px-5 py-4 align-top">
                        ...
                        <td className="px-5 py-4 align-top">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => archiveDisc.open(doc)}>
      ...
      <ConfirmDialog
        ...
        message={`Archive "${archiveDisc.payload?.title ?? 'this document'}"? This removes it from active classified records and stores it in the archive log.`}
