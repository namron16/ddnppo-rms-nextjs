    ...
    <tr className="border-b border-slate-100 hover:bg-slate-50/80 transition group">
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
          ...
          <button className="text-[10px] font-semibold px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition">
            👁
          </button>
        </div>
      </td>
...
// 201 CHECKLIST MODAL
// ─────────────────────────────────────────────
function Checklist201Modal({ person, onClose }: {
  ...
  // Explicitly typed arrays — prevents TS "unknown key" errors in .map()
  // "Name" row not shown — title bar shows "Police Personal File"; name appears as first leftRow in the card.
  const leftRows: { label: string; value: string }[] = [
    { label: 'Unit',               value: person.unit },
    { label: 'Address',            value: person.address          ?? '—' },
    { label: 'Contact No.',        value: person.contactNo        ?? '—' },
    { label: 'Date of Retirement', value: person.dateOfRetirement ?? '—' },
    { label: 'Status',             value: person.status           ?? 'Active' },
    { label: 'File Created',       value: formatDate(person.dateCreated) },
  ]

  const rightRows: { label: string; value: string }[] = [
    { label: 'Firearm Serial No.',  value: person.firearmSerialNo  ?? '—' },
    { label: 'Pag-IBIG No.',        value: person.pagIbigNo        ?? '—' },
    { label: 'PhilHealth No.',      value: person.philHealthNo     ?? '—' },
    { label: 'TIN',                 value: person.tin              ?? '—' },
    { label: 'Payslip Account No.', value: person.payslipAccountNo ?? '—' },
  ...
  return (
    <Modal
      open={!!person}
      onClose={onClose}
      title="Police Personal File"
      width="max-w-5xl"
    >

      {/* ══════════════════════════════════════════════
          RESUME-STYLE ID CARD HEADER
          Layout: [Photo] | [Left cols] | [Right cols] | [Actions]
      ══════════════════════════════════════════════ */}
      <div className="border-b border-slate-200">

        {/* Dark navy banner — photo + two full-width info columns, no buttons here */}
        <div className="bg-[#0f1c35] px-6 py-5 flex items-stretch gap-5">

          {/* Photo — fixed square, larger */}
          <div className="flex-shrink-0">
            <div
              className="w-40 h-40 rounded-xl border-2 border-white/20 flex flex-col items-center justify-center text-center"
          ...
          {/* Left info column — full flex space, no cramping */}
          <div className="flex-1 flex flex-col gap-2 justify-center min-w-0">
            {leftRows.map(row => (
              <div key={row.label} className="flex items-baseline gap-2 min-w-0">
                <span className="text-[9.5px] text-white/45 font-semibold uppercase tracking-wide whitespace-nowrap w-28 flex-shrink-0">
                  {row.label}:
                </span>
          ...
          {/* Right info column — full flex space */}
          <div className="flex-1 flex flex-col gap-2 justify-center min-w-0">
            {rightRows.map(row => (
              <div key={row.label} className="flex items-baseline gap-2 min-w-0">
                <span className="text-[9.5px] text-white/45 font-semibold uppercase tracking-wide whitespace-nowrap w-32 flex-shrink-0">
                  {row.label}:
                </span>
                <span className="text-[12px] text-white font-medium truncate">
                  {row.value}
        ...
        <div className="bg-slate-50 px-6 border-t border-slate-200">
          ...
          {/* Row 2: stat chips left, buttons right */}
          <div className="flex items-center gap-2 py-2">
            <div className="flex gap-1.5 flex-wrap text-xs font-semibold">
              <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">✅ {complete} Complete</span>
              <span className="bg-red-100    text-red-700    px-2 py-0.5 rounded-full">❌ {missing} Missing</span>
              <span className="bg-amber-100  text-amber-700  px-2 py-0.5 rounded-full">🔄 {forUpdate} For Update</span>
              <span className="bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">⚠️ {expired} Expired</span>
              <span className="text-slate-400 px-1 py-0.5 text-xs">{person.documents.length} items total</span>
            </div>
            <div className="flex items-center gap-1.5 ml-auto">
              <button className="text-[11px] font-medium px-2.5 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition whitespace-nowrap">
                📤 Batch Upload
              </button>
              <button className="text-[11px] font-medium px-2.5 py-1 bg-slate-200 text-slate-700 rounded-md hover:bg-slate-300 transition whitespace-nowrap">
                🖨 Print
              </button>
              <button className="text-[11px] font-medium px-2.5 py-1 bg-slate-200 text-slate-700 rounded-md hover:bg-slate-300 transition whitespace-nowrap">
                ✏️ Edit
        ...
        {(missing + expired + forUpdate) > 0 && (
          <div className="px-6 py-2 border-t border-amber-100 bg-amber-50">
            <div className="flex items-center gap-2 text-amber-800">
              <span className="text-xs flex-shrink-0">⚠️</span>
              <span className="text-xs">
                This 201 file has <strong>{missing + expired + forUpdate}</strong> document(s) that require attention before submission to DPRM.
              </span>
            </div>
          </div>
        )}
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className={cls} placeholder="PN-2024-0001"
              value={form.serialNo} onChange={e => f('serialNo', e.target.value)} />
          </div>
        </div>
        <div>
          <label className="block text-[11px] font-semibold uppercase tracking-widest text-slate-500 mb-1.5">
          ...
          <input className={cls} placeholder="e.g. DDNPPO HQ, PCADU"
...
export default function PersonnelFilesPage() {
  ...
  // Cast keys explicitly so useSearch<T> (unconstrained generic) resolves correctly
  ...
  // Typed explicitly — prevents "unknown" inference on .map(s => s.label) key prop
  const statCards: { icon: string; value: number; label: string; bg: string; num: string }[] = [
    { icon: '👥', value: PERSONNEL_201.length,                                                        label: 'Personnel Records',  bg: 'bg-blue-50',    num: 'text-blue-700'    },
        ...
        <div className="bg-white border-[1.5px] border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                PNP DPRM 201 File — Checklist in the Updating of Records (Police Personal File Database)
              </p>
            </div>
          ...
          <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50">
            ...
            <span className="text-xs text-slate-400 ml-auto">
              {filtered.length} of {PERSONNEL_201.length} personnel
            </span>
          </div>
